...
QUnit.test('Testing Table function with several sets of inputs', function (assert) {
    assert.equal(getTableNumber(10),10 , 'Tested with the table number: If it is between 10 to 100 take the given value');
    assert.equal(getTableNumber(120), 100, 'Tested with table number greater than hundred. If the table number is greater than 100 take it as 100.');
    assert.equal(getTillWhere(22), 20, 'Tested with Table till value greater than twenty and if it is greater than twenty it should take it as twenty');
    assert.throws(function () { getTableNumber('a'); }, /The given argument is not a number/, 'Passing in a string correctly raises an Error');
});
